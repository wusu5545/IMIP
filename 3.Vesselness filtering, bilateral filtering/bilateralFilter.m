function [ filtered ] = bilateralFilter( im,maskSize,sigmaC,sigmaS )
...
for y=msh+1:size(im,1)-msh
    for x=msh+1:size(im,2)-msh
        ...
        intensityDifferences = maskPixels - maskPixels(msh,msh)*ones(maskSize,maskSize);
        ...
        filtered(y,x)= sum(maskPixels.*weights)/sum(weights);
