...
% fy = ???;
fx = conv2(im,DoGMask,'same');
fy = conv2(im,DoGMask','same');
...
Qxx = conv2(Jxx,gaussMaskrho,'same');               
Qyy = conv2(Jyy,gaussMaskrho,'same');               
Qxy = conv2(Jxy,gaussMaskrho,'same');      
...
for s=1:m
    for t=1:n
        % Build the structure tensor matrix (variable: part(2x2)) iteratively
         part(1,1) = Qxx(s,t);
         part(1,2) = Qxy(s,t);
         part(2,1) = Qxy(s,t);
         part(2,2) = Qyy(s,t);
        ...
        if(e1(s,t)<thres && e2(s,t)<thres)
            ...
            homo(s,t) = 1;
        elseif(e1(s,t)>=thres && e2(s,t)<thres)
            disp('edge');
            edg(s,t) = 1;
        elseif(true)
            disp('corner');
